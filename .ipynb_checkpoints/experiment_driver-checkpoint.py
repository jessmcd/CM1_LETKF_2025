...
ensN=36                          # ensemble size
base_dir = "/scratch/home/jessica.mcdonald/LETKF_runs/DBZ_VR"  # name/location of the directory where all of your model output will go (it doesn't need to already exist)
#base_dir = "/work/jessica.mcdonald/test"

nthreads = 1            # number of threads for run_filter.py (the actual DA)
ncores   = 64           # the number of cores that CM1 will use to run in run_fcst.py

init_sounding = "soundings"    # location of soundings for each ensemble (sounding name should match ensemble name (i.e. member 001 gets sounsding/Run01Sounding.txt for input_sounding))
obs_loc = 'Obs/8may24_cm1_obs.csv' # location of radar files
obs_inc = [ 'DBZ', "VR"]            # options: VR, DBZ, or all (for any available data in data file). 
...
run_setup     = False              # set this to false if you have already run the cook period (or already have those files) and ONLY want to do a DA experiment 
...
# --------------------------------------------------------------------------
# Initialize the run, use random warm bubbles

if run_setup:

    print('Beginning model set up and cook period')
    model_start = (DA_start_time - dt.timedelta(minutes=cook_period)).strftime('%Y,%m,%d,%H,%M,%S')  # model start time is da_start_time - cook period, in YYYY,mm,dd,HH,MM,SS format

    # set up experiment file and create all of the base states
    os.system(f"python create_run_letkf.py  -n {ensN} -b {base_dir} --model_start {model_start} --init_sounding {init_sounding} --obs_loc {obs_loc}") 
    os.system(f"python run_fcst.py          -e {exp_name} --ncores {ncores} -i") 
    os.system(f"python ens.py               -e {exp_name} --init0 -t {model_start} --write ")
      
    # cook period!
    os.system(f"python run_fcst.py -e {exp_name} --run_time {cook_period*60} -t {model_start} --ncores {ncores}")
...
for time in times:
    ...
    os.system(f"python run_filter.py -e {exp_name} -t {time} --freq -{assimilation_freq*60} --nthreads {nthreads} --included_obs {obs_inc_list}" ) 

    if time != times[-1]: #do the pure forecast at the end, not forward integration to next DA cycle
        os.system(f"python run_fcst.py -e {exp_name} --run_time {assimilation_freq*60} -t {time} --ncores {ncores}" )

#--------------------------------------------------------------------------
# Make a forecast
if forecast_length > 0:
    time = times[-1].astype(dt.datetime).strftime('%Y,%m,%d,%H,%M,%S')
    os.system(f"python run_fcst.py -e {exp_name} --run_time {forecast_length*60} -t {time} --ncores {ncores}")
