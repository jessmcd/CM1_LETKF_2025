...
# get kind then setup type and units
  ...
  ltypes = N.empty([obs,stringlen], dtype=str)
...
#        to return a the standard set of data sorted by x/y/z/sec/kind to the calling routine
  ...
  if sort:
    array = N.recarray((x.size,), dtype = [('x', N.float64), ('y', N.float64), ('z', N.float64), \
                                           ('lat', N.float64), ('lon', N.float64), ('secs', int), \
                                           ('kind', int),  ('index', int), ('outlier',N.float64)])
    array['x']       = x
    array['y']       = y
    array['z']       = z
    array['lat']     = lat
...
# Create fake data....
    ...
    kind   = N.array((11,12), dtype = int32)
    status = N.array((1,1), dtype = int32)
