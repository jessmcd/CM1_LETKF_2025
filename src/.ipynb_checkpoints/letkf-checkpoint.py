...
# Parse input command options
   ...
   # parser.add_option(      "--prior_inflate",  dest="prior_inflate",  type = "int",   help = "Sets type of adaptive inflation (0,1,2)", default=None)
   # parser.add_option(      "--post_inflate",  dest="post_inflate",  type = "int",   help = "Sets type of posterior inflation, such as RTPP (0,1,2,3,4)", default=None)
...
# Get experiment file
    ...
    if options.exper == None:
        ...
        with open(options.exper, 'rb') as p: exper = json.load(p)
    ...
    for key in list(obs_diag.keys()):
        ...
        if d.size > 0:
            ...
            print(("\n -->  LETKF:  %s  NOBS: %5.5d    %3.3s: %3.1f  RMSI: %6.3f  M-Innov: %7.3f  Spread: %6.3f  CRatio: %7.4f " \
                    % (analysis_time.strftime("%Y-%m-%d_%H:%M:%S"), np.sum(index_kind), obs_diag[key][0], np.sqrt(ob_err), \
...
# Adaptive inflation: read in or create dummy file....
    ...
    if prior_inflate == 0:   # no adaptive inflation, just create a dummy array of ones...
        print("\n -->  LETKF:  Adaptive inflation is turned off....\n")
        inflate_array = np.ones((state.nz, state.ny, state.nx), order='F', dtype=np.float64)
        
    if prior_inflate >= 2:   # fixed inflation
        print(f"\n -->  LETKF:  No Adaptive inflation, using Fixed Inflation value of {prior_inflate_value} \n")
        inflate_array = prior_inflate_value * np.ones((state.nz, state.ny, state.nx), order='F', dtype=np.float64)
            
    if prior_inflate == 1:   # adaptive inflation 
        ...
        else: # this occurs at the start of every experiment with adaptive inflation
...
# Begin LETKF analysis
    ...
    #         if np.sum(index_kind) > 0:
    #             print(("\n-->  LETKF:  %d observations of %3.3s is now being assimilated!\n" % (np.sum(index_kind), obs_diag[key][0])))
    ...
    #                                             rhoriz, rvert, rtime, nthreads, cutoff, zcutoff, update_theta, aInflate, inflate_array,
