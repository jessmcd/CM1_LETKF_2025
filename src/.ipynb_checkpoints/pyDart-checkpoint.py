...
def nice_mxmnintvl( dmin, dmax, outside=False, max_steps=20, cint=None, sym=False):
    ...
    if cint == None or cint == 0.0:
        try:
            ...
            return am1[index], ax1[index], np.linspace(am1[index], ax1[index], cints[index])
        except IndexError:
            return None, None, np.zero(1)
    else:
        return am1, ax1, np.arange(am1, ax1+cint, cint)
#===============================================================================        
...
# Build a list of search conditions
        ...
        if start != None:
            utime_start = cftime.date2num(self.start, sec_utime) #sec_utime.date2num(self.start) JEDIT
            cond.append( "(" + str(utime_start) + " <= utime)" )
        
        if end != None:
            utime_end   = cftime.date2num(self.end, sec_utime) #sec_utime.date2num(self.end) JEDIT
...
#      (see http://sourceforge.net/project/showfiles.php?group_id=80706&package_id=142792)
        ...
        xy_unique, index = np.unique(xy, return_index=True)
        ...
        x   =  np.where(x > 180., x - 360., x)  # make west longitudes...
        z   =  np.array(z, dtype="float32")

        el = np.round(data['elevation'].mean(),2)
...
# Now, call grid to grid some data
        ...
        if variable.upper() == "DBZ" or variable.upper() == "REFLECTIVITY":
            ...
            zimask = np.where( zi > 10., zi, np.nan)
...
# Later, we might add an error check here to make sure the data are all the same kind
        ...
        if clear_air != -1:
            x0 = clear_air['lon'][:]
            y0 = clear_air['lat'][:]      
            x0 =  np.where(x0 > 180., x0 - 360., x0)  # make west longitudes...
            z0 =  clear_air['value'][:]
        ...
        if dbz != -1:
            x1 = dbz['lon'][:]
            y1 = dbz['lat'][:]       
            x1 = np.where(x1 > 180., x1 - 360., x1)  # make west longitudes...
            z1 = dbz['value'][:]
            el = dbz['z'][0]
            time = dbz['date'][0]
...
# Open DART PyTables file
        ...
        if len(self.index) == 0 and all ==False:
            self.index = np.arange(table.nrows)
...
# Open DART PyTables file
        ...
        if type(variable) == type("str"):       # Determine the user's request data type
            ...
            if len(self.index) != 0:
                ...
                lat    = data['lat']
                lon    = data['lon']
                x      = np.where(data['x'] != _missing, data['x']/1000., _missing)
                y      = np.where(data['y'] != _missing, data['y']/1000., _missing)
                z      = np.where(data['z'] != _missing, data['z']/1000., _missing)
                az     = np.where(data['azimuth']   != _missing, data['azimuth'],   _missing)
                el     = np.where(data['elevation'] != _missing, data['elevation'], _missing)
...
# Open DART PyTables file
        ...
        if len(self.index) != 0:
            ...
            if dx < 0.001: dx = np.max(np.diff(x))
            
            y      = np.sort(np.unique(y))
            dy     = np.median(np.diff(y))
            if dy < 0.001: dy = np.max(np.diff(y))
            ...
            print(("%s  Min/Max    Y (km):   %10.4f   %10.4f  DY(km): %5.2f (values are approximate)" % (variable,y.min(), y.max(), np.median(dy))))
...
# Create group for misc file header information
        ...
        print("\n")
        for numcp in np.arange(num_copies+num_qc):      # Now read the observation, and if provided, the truth value
...
# Read in the obs sequentially!
        ...
        while n < group_header.attributes.cols.num_obs[0]:
            ...
            stuff            = fi.readline()
            stuff            = stuff.split()
            row["lon"]       = np.rad2deg(read_double_precision_string(stuff[0]))
            row["lat"]       = np.rad2deg(read_double_precision_string(stuff[1]))
...
# Check to see if this GEOS cloud pressure observation
            ...
            if row["kind"] == ObType_LookUp("GOES_CWP_PATH"):
                stuff = fi.readline()
                stuff = stuff.split()
                row["satellite"][0] = np.rad2deg(read_double_precision_string(stuff[0]))
                row["satellite"][1] = np.rad2deg(read_double_precision_string(stuff[1]))
                stuff = fi.readline().split()
                row["satellite"][2] = np.float(stuff[0])
...
# Check to see if its radial velocity and add platform information...need BETTER CHECK HERE!
            ...
            if row["kind"] == ObType_LookUp("VR"):
                ...
                stuff                      = fi.readline()
                stuff                      = stuff.split()
                row['platform_lon']        = np.rad2deg(read_double_precision_string(stuff[0]))
                row['platform_lat']        = np.rad2deg(read_double_precision_string(stuff[1]))
                ...
                elevation_angle = np.arcsin(row['platform_dir3']) 

                row['elevation']         = np.rad2deg(elevation_angle)
...
# Compute some coordinates 
                if radar_loc != None:
                    ...
                    R = np.sqrt(row['x']**2 + row['y']**2)
                    elevation_angle = beam_elv(R, row['z'])
                            
                    row['platform_dir1']     = (row['x'] / R) * np.deg2rad(elevation_angle)
                    row['platform_dir2']     = (row['y'] / R) * np.deg2rad(elevation_angle)
                    row['platform_dir3']     = np.sin(np.deg2rad(elevation_angle))
...
# Add in a full date and time string to data set, as well as create a UTIME in seconds for searching
            ...
            date = cftime.num2date(days, day_utime) #day_utime.num2date(days) JEDIT
            
            row['utime']     = round(cftime.date2num(date, sec_utime)) # JEDIT round(sec_utime.date2num(date)) #  to prevent sometimes truncating down to next integer
...
#-------------------------------------------------------------------------------
    ...
    def dict2hdf(self,obs_dict,filename=None,radar_loc=None):
        ...
        if radar_loc != None:
            print(("PyDart.dict2hdf:  radar location supplied:  lat: %f  lon: %f " % (np.degrees(radar_loc[0]),np.degrees(radar_loc[1]))))
...
# Read in the obs sequentially!
        ...
        for n in np.arange( obs_dict['counter'] ):
...
# Need to switch the lat and lon these around to be correct in version 2 of pyDART...
                ...
                row['elevation']           = np.rad2deg(np.arcsin(obs_dict['platform_dir3'][n]))
            
                if np.abs(np.cos(row['elevation'])) < 0.001:   # pointing straight up?
                    row['azimuth'] = 0.0
                else:
                    row['azimuth'] = np.rad2deg( np.arctan2( row['platform_dir1'] / np.cos(np.deg2rad(row['elevation'])), \
                                                           row['platform_dir2'] / np.cos(np.deg2rad(row['elevation'])) ) )
...
# Add in a full date and time string to data set, as well as create a UTIME in seconds for searching
            ...
            row['date']      = str(date)
            row['utime']     = round(cftime.date2num(date, sec_utime)) #JEDIT
            ...
            if n % 5000 == 0:
                ...
                print((" Date,sec_utime,utime = ", date,cftime.date2num(date, sec_utime), row['utime']))
...
#  Open netcdf files, read data, flatten, and append
  ...
  # if dbz_max   - finally, as a last step, clip the reflectivity to be less than dbz_max
            ...
            try:
                ...
                dbz = np.where(dbz < dbz_missing_fudge+0.1, f.MissingData, dbz)
            except KeyError:
...
# Figure out the bounding box indices to make the output go a LOT faster
            ...
            lindex = np.arange(lats.size)[index]
            index  = (lons > lon_bbox[0] ) & (lons < lon_bbox[1])
            mindex = np.arange(lons.size)[index]
            index  = (hgts >= hgt_bbox[0] ) & (hgts < hgt_bbox[1])
            kindex = np.arange(hgts.size)[index]
...
# Now process the data (note, 2D arrays still have a 'k'-index, the k-loop executes once)
            ...
            # Add in a full date and time string to data set, as well as create a UTIME in seconds for searching
                            ...
                            row['days']    = np.int(gc)
                            row['seconds'] = (gc - np.int(gc)) * 86400
...
# Code (from RLT) to output correct index number for each ob in the output DART file
            ...
            fi.write("    %20.14f          %20.14f          %20.14f\n" % (np.radians(row["lon"]), np.radians(row["lat"]), row["height"]) )
...
# Check to see if its radial velocity and add platform information...need BETTER CHECK HERE!
            ...
            if row["kind"] == ObType_LookUp("VR"):
                ...
                platform_lat, platform_lon = np.radians(row["platform_lat"]), np.radians(row["platform_lon"])
