...
# Input is from a COMMAS model input file
        ...
        if self.filename.count(".commas") or self.filename.count(".sound") or self.filename.count(".txt") \
            ...
            self.psfc, self.tsfc, self.qsfc = np.fromfile(self.filename, dtype='float', sep=" ",count=3)   # Load first line with surface values
            ...
            data = np.loadtxt(self.filename, skiprows=1)    # Load the rest of the file, skipping first line (nice function!)
            ...
            self.u  = data[:,3]
            self.v  = data[:,4]
            self.nz = np.size(self.zc)
            self.pi = np.zeros((self.nz),dtype='float')
            self.tv = np.zeros((self.nz),dtype='float')
            self.ri = np.zeros((self.nz),dtype='float')
         ...
         # Check for .txt files
            ...
            for n in np.arange(self.nz-1):      # Integrate the pi hydrostatic equation to get pressure
                dz    = self.zc[n+1] - self.zc[n]
                self.pi[n+1] = self.pi[n] - 9.806 / (0.5*(self.tv[n+1]+self.tv[n])*1004./dz)

            for n in np.arange(self.nz-1):      # Compute the Richardson number based on virtual temperature
                dz    = self.zc[n+1] - self.zc[n]
            ...
            e       = np.clip( self.qv*self.p/(0.622+self.qv), 1.0e-5, 1000.)
            e       = np.log(e/6.112)
            self.td = Czero + 243.5 / ( 17.67/e - 1.0 )                 # Bolton's approximation        
            self.spd = np.sqrt( self.u[:]**2 + self.v[:]**2 )
            self.dir = 270.0 - 57.29578*np.arctan2(self.v[:], self.u[:])
...
# Read from raw2 file from Univ of Wyoming
            ...
            data = np.loadtxt(self.filename, dtype='float')    # Load the file
            ...
            self.pi = np.zeros((self.nz),dtype='float')
            self.tv = np.zeros((self.nz),dtype='float')
            self.th = np.zeros((self.nz),dtype='float')
            self.ri = np.zeros((self.nz),dtype='float')

            self.u  = self.spd * np.cos(np.deg2rad(-self.dir + 270.))
            self.v  = self.spd * np.sin(np.deg2rad(-self.dir + 270.))
...
# Read point forecast sounding from NSSL WRF
            ...
            data = np.loadtxt(self.filename, delimiter = ",", dtype='float',skiprows=8)    # Load the file
            ...
            esat = p1013*10.**(10.79586*(1-Czero/(self.t))-5.02808*np.log10((self.t)/Czero)+ \
                1.50474e-4*(1.-10**(-8.29692*((self.t)/Czero-1.)))+ \
                0.42873e-3*(10.**(4.76955*(1.-Czero/(self.t)))-1.)-2.2195983)

            es   = p1013*10.**(10.79586*(1-Czero/(self.td))-5.02808*np.log10((self.td)/Czero)+ \
...
# Read from GAUS sounding file
            ...
            p,tc,rh,dir,spd,z = np.loadtxt(self.filename, usecols=(5,6,7,8,9,19), unpack=True, skiprows=6)    
            ...
            self.qv = rh / 100. * 3.8 * np.exp(17.27*tc/(tc+237.16)) / p
            self.u  = spd * np.cos(np.deg2rad(270.0 - dir))
            self.v  = spd * np.sin(np.deg2rad(270.0 - dir))
...
# Read from commas netcdf file
            ...
            if self.x == 0:                             # We plot the base state sounding....
                ...
                x0 = np.nonzero( np.where(self.xc <= self.x,True,False) )
                y0 = np.nonzero( np.where(self.yc <= self.y,True,False) )
                t0 = np.nonzero( np.where(self.tc <= self.time,True,False) )
...
# Debug code....
    ...
    def write(self, filename=None, sharpPY=None, sheader=None):
        ...
        if sharpPY == None:
#           np.savetxt(filename, np.transpose((self.p, self.zc, self.th, self.qv*1000., self.u, self.v)), fmt='%8.2f')
            np.savetxt(filename, np.transpose((self.zc, self.th, self.qv*1000., self.u, self.v)), fmt='%8.2f')
            f = open(filename, 'r')
            text = f.read()
            f.close()
            f = open(filename, 'w')
...
# Make the hodograph background plot circles - from Shea wind rose
        MaxNumCircles = np.fix(maxwind/circFr + 1.)
        FrMaxNum      = circFr*MaxNumCircles   # max circle radius
        nCirc = np.arange(361)
        xCirc = np.zeros((MaxNumCircles,361), dtype='float')
        yCirc = np.zeros((MaxNumCircles,361), dtype='float')
        rad   = 4.*np.arctan(1.0)/180.            # degress to radians
        xcos  = np.cos(nCirc[:]*rad)*circFr
        xsin  = np.sin(nCirc[:]*rad)*circFr
        for n in np.arange(MaxNumCircles):        # plot coordinates for freq circles
            xCirc[n,:] = (n+1)*xcos[:]
            yCirc[n,:] = (n+1)*xsin[:]
...
#       np.polyline (wks,hodo_bkgd,[-FrMaxNum-extraSpace, FrMaxNum+extraSpace], [0.0, 0.0], hodoOpts)
#       np.polyline (wks,hodo_bkgd,[0.0,0.0],[-FrMaxNum-extraSpace, FrMaxNum+extraSpace], hodoOpts)
...
# Add in height markers
        ...
        for z in hodoLabelHeights:
            ...
            if z < 1000.:
                label = "%d m" % np.fix(self.zc[0]) 
            else:
                label = "%d km" % np.fix(z/1000.)
...
#                  ("        %d\n" % (self.nz)))
        ...
        np.savetxt('tempfile', np.transpose((self.zc[::-1]+sfcz, self.th[::-1], sh[::-1], self.u[::-1], self.v[::-1])), fmt='%12.6f')
        f2 = open('tempfile', 'r')
        text = f2.read()
        f2.close()
        f.write(header)
