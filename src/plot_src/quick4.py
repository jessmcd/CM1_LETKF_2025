...
# get coordinates for contour plots
    ...
    plot    = map.contourf(xx, yy, np.ma.masked_less_equal(dbz,_min_dbz), clevels, cmap=_ref_ctable)
    ...
    scale_w_clevels = min(max(np.int(height/1000.), 1.0), 7.0)
    clevels = scale_w_clevels*np.arange(-15.,16.,1.)
    wmask   = np.ma.masked_array(w, mask = [np.abs(w) <= scale_w_clevels*_min_w])
