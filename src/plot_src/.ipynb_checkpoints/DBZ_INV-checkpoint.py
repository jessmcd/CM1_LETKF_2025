...
if __name__ == "__main__":
    ...
    IN_TZ = np.zeros((zbins.size,nbins))
    IN_T  = np.zeros((nbins))
    IN_Z  = np.zeros((zbins.size))
    m = -1
    
    fig = P.figure(figsize=(12,12))
    fig.text(0.72, 0.72, "\n\nInnovations", size=20, va="baseline", ha="center", multialignment="center", weight='bold')
        ...
        if n % bin_delta == bin_delta-1:
            ...
            for k in np.arange(zbins.size-1):
                ...
                if np.sum(index == True) > 2: 
                    ...
                    np.sqrt(fcst_msi), d.mean(), np.sqrt(obs_var[1]**2 + sdHxf**2), consi_ratio))
...
# Plotting
    ...
    if auto_clevels:
        cmin, cmax, cint, clevels = nice_clevels(-15., 15, outside=False, cint = 3.0)
    else:
        clevels = np.arange(spread_limits[0], spread_limits[1], spread_limits[2])

    cs1=axC.contourf(datebins, zbins/1000., IN_TZ, clevels, cmap='YlOrRd')
