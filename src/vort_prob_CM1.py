...
import plot_src.cbook2 as cbook
...
def nearest_N_prob(im, n):
    ...
    return( np.where(improc > 0.0, 1.0, 0.0) )
...
def CM1_get_Wz(files, exper):
  ...
  wz_ens = np.zeros((ens_size,level,exper['cm1namelist'][0][2],exper['cm1namelist'][1][2]))
...
vvort  = np.zeros((ntimes,ens_size,level,ny,nx))
vvmean = np.zeros((ntimes,ens_size,ny,nx))
...
if xplot:
  x_swath = xplot[0] + xplot[2]*np.arange(1+np.int((xplot[1]-xplot[0])/xplot[2]))
  y_swath = yplot[0] + yplot[2]*np.arange(1+np.int((yplot[1]-yplot[0])/yplot[2]))
else:
  x_swath = xc
  y_swath = yc
  
vort   = np.zeros((x_swath.size,y_swath.size), order='F')
xx, yy = np.meshgrid(xc, yc)
  
for n in np.arange(ens_size):
  ...
  for m in np.arange(ntimes):
    ...
    if m == 0:
      ...
      xobs   = np.concatenate((xobs,xx.flatten()))
      yobs   = np.concatenate((yobs,yy.flatten()))
      obs    = np.concatenate((obs,vvmean[m,n].flatten()))
      xobs2  = np.concatenate((xobs,xx.flatten()))
      yobs2  = np.concatenate((yobs,yy.flatten()))
      obs2   = np.concatenate((obs,vvmean[m,n].flatten()))
